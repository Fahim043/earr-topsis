...
The implementation follows the same conceptual pipeline across methods: data loading, decision-maker aggregation, normalization, weighting, ideal-solution calculation, distance calculation, closeness computation, and ranking. The difference among methods is how the decision makers are selected or weighted before the TOPSIS calculation.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.85\textwidth]{m1_normal_topsis.png}
    \caption{M1 classical fuzzy TOPSIS architecture.}
    \label{fig:m1-architecture}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.85\textwidth]{m2_bagged_topsis.png}
    \caption{M2 bootstrap bagged fuzzy TOPSIS architecture.}
    \label{fig:m2-architecture}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.85\textwidth]{m4_weighted_bagging.png}
    \caption{M4 reliability-weighted bagging architecture.}
    \label{fig:m4-architecture}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.85\textwidth]{m6_reliability_weighted.png}
    \caption{M6 reliability-weighted aggregation architecture.}
    \label{fig:m6-architecture}
\end{figure}

\section{Algorithmic Summary of the Proposed Methods}

The three proposed methods can be summarized as follows.

\subsection{M4 Algorithm}

\begin{enumerate}
    \item Load the fuzzy decision matrix.
    \item Compute decision-maker reliability scores \(R_k\).
    \item Convert reliability scores into sampling probabilities.
    \item Generate bootstrap bags by sampling decision makers with replacement using reliability-weighted probabilities.
    \item Run fuzzy TOPSIS on each bag.
    \item Average closeness coefficients across bags and rank alternatives.
\end{enumerate}

\subsection{M6 Algorithm}

\begin{enumerate}
    \item Load the fuzzy decision matrix.
    \item Compute decision-maker reliability scores \(R_k\).
    \item Generate bootstrap bags using uniform sampling.
    \item Inside each bag, aggregate triangular fuzzy ratings using reliability-weighted means.
    \item Run fuzzy TOPSIS on each bag.
    \item Weight bag outputs using bag reliability and compute the final ranking.
\end{enumerate}

\subsection{M7 Algorithm}

\begin{enumerate}
    \item Flatten each decision maker's full fuzzy rating tensor.
    \item Compute entropy reliability.
    \item Compute variance-consistency reliability.
    \item Compute clone/agreement-pattern reliability.
    \item Combine the reliability signals into a final reliability score.
    \item Sample decision makers using reliability-weighted bootstrap sampling.
    \item Aggregate ratings inside each bag using reliability-weighted fuzzy aggregation.
    \item Run fuzzy TOPSIS on each bag.
    \item Weight bags by collective reliability and produce final closeness coefficients.
\end{enumerate}

\section{Code Mapping}

The implementation files map to the thesis methods as follows:

\begin{table}[H]
\centering
\small
\caption{Mapping between thesis methods and implementation files.}
\label{tab:code-mapping}
\begin{tabular}{lll}
\toprule
Method & File & Role \\
\midrule
M1 & \texttt{m1\_normal\_topsis.py} & Classical fuzzy TOPSIS baseline \\
M2 & \texttt{m2\_bagged\_topsis.py} & Corrected bootstrap baseline \\
M3 & \texttt{m3\_ml\_filtered.py} & Reliability filtering comparator \\
M4 & \texttt{m4\_weighted\_bagging.py} & Proposed reliability-weighted bagging \\
M5 & \texttt{m5\_cluster\_stratified.py} & Cluster-stratified ablation branch \\
M6 & \texttt{m6\_reliability\_weighted.py} & Proposed reliability-weighted aggregation \\
M7 & \texttt{m7\_entropy\_reliability.py} & Final \methodname{} method \\
\bottomrule
\end{tabular}
\end{table}
